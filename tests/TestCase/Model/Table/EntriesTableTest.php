<?php
	...
	class EntriesTest extends SaitoTableTestCase {
		...
		public function testCreateSuccessNewThread() {
			...
			//= Setup CurrentUser
            $SaitoUser = new SaitoUser(['id' => 100, 'username' => 'foo', 'user_type' => 'admin']);
			Registry::set('CU', $SaitoUser);

			// +1 because str_pad calculates non ascii chars to a string length of 2
			$subject = str_pad(
				'Sübject', $this->Table->setting('subject_maxlength') + 1, '.'
			);
			$data = [
				'pid' => 0,
				'subject' => $subject,
        ...
        public function testCreateAllowanceAnswer() {
            ...
            $this->assertFalse($this->Table->createPosting($thread));

            /**
             * admin thread allowed
             */
            $SaitoUser->setSettings($admin);
            $this->assertNotEmpty($this->Table->createPosting($thread));

            /**
             * user answer allowed
             */
            $SaitoUser->setSettings($user);
            $this->assertNotEmpty($this->Table->createPosting($answer));
        ...
        public function testCreateAllowanceThread() {
             ...
             */
            $SaitoUser->setSettings($user);
            $this->assertFalse($this->Table->createPosting($answer));
		...
		public function testDeleteNodeCompleteThread() {
			...
			$this->Table->Categories = $this->getMock(
				'Categories', ['updateThreadCounter']
			);
			$this->Table->Categories->expects($this->once())
		...
		public function testAnonymizeEntriesFromUser() {
			...
			$expected = 0;
			$result = $this->Table->find()->where(['edited_by' => 'Ulysses'])->count();
			$this->assertEquals($result, $expected);
