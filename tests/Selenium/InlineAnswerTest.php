...
class InlineAnswerTest extends SaitoSeleniumTestCase
{
        ...
        for ($second = 0;; $second++) {
            ...
            try {
                if (!$this->isElementPresent(
                    "css=.threadLeaf[data-id={$id}]  .entry.reply"
                )
                ) {
                    break;
                }
    ...
    protected function _createNewInlineAnswer($parentId)
    {
        ...
        $this->assertTrue(
            $this->isVisible(
                "css=.threadLeaf[data-id={$parentId}] .panel-footer"
            )
        );
        ...
        for ($second = 0;; $second++) {
            ...
            try {
                if ($this->isVisible(
                    "css=.threadLeaf[data-id={$parentId}] .entry.reply"
                )
                ) {
                    break;
                }
        ...
        $this->type(
            "css=.threadLeaf[data-id={$parentId}]  #EntrySubject",
            "Id: {$this->nextId}"
        );
    ...
    {
        $this->waitForVisibleJq(
            ".threadLeaf[data-id={$id}] .js-entry-view-core"
        );
    }
    ...
    {
        $this->waitForVisibleJq(
            ".threadLeaf[data-id={$id}] .threadLeaf-content"
        );
    }
