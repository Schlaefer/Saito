<?php Configure::write('Asset.timestamp', false); ?>
<script>
	window.resourcesFolderName = '<?= $this->Html->assetUrl('js/forum_disabled_Resources')?>';
</script>
<style type="text/css">
	...
	#top {
		...
		width: 300px;
		background: none;
		background-image: url('<?= $this->Html->assetUrl('img/bubbles_right.png') ?>');
	...
	#top2 {
		height: 100%;
		width: 100%;
		background-image: url('<?= $this->Html->assetUrl('img/bubbles_outer_large.png'); ?>');
		position: fixed;
		z-index: 5;
	}
</style>
