...
class MapHelper extends AppHelper
{
    ...
    {
        $this->_apiEnabled = (bool)Configure::read(
            'Saito.Settings.map_enabled'
        );
        if (!$this->_apiEnabled) {
            return;
        }
    ...
    public function map($users, array $options = [])
    {
        ...
        foreach ($users as $key => $user) {
            ...
            if ($edit) {
                ...
                $usersForMap[$key]['lat'] = round($usersForMap[$key]['lat'], $this->_precision) + $rand;
                $usersForMap[$key]['lng'] = round($usersForMap[$key]['lng'], $this->_precision) + $rand;
            }
        }
