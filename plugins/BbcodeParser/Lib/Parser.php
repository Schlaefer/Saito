...
class Parser extends \Saito\Markup\Parser
{
    ...
    protected function _initParser(&$options)
    {
        ...
        $this->_Preprocessors->add(
            new Processors\BbcodeImageUploadLegacyPreprocessor()
        );
        ...
        $this->_Postprocessors->add(
            new Processors\BbcodeQuotePostprocessor($options)
        );
        ...
        foreach ($this->_tags[$set] as $definition) {
            ...
            switch ($definition['type']) {
                case 'replacement':
                    ...
                    break;
                case 'class':
                    require_once(\Cake\Core\Plugin::path(
                            'BbcodeParser'
                        ) . DS . 'Lib' . DS . 'jBBCode' . DS . 'Definitions' . DS . 'JbbCodeDefinitions.php');
                    $class = '\Plugin\BbcodeParser\Lib\jBBCode\Definitions\\' . ucfirst(
                            $title
                        );
                    $this->_Parser->addCodeDefinition(
                        new $class($this->_Helper, $options)
                    );
                    break;
                default:
                    throw new \Exception();
